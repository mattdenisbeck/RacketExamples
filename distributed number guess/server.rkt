...
(struct theNumber (x) #:mutable #:transparent)
(define choice (theNumber 0))
...
(define (connect u client)
  (if (false? u)
      ...
      (make-bundle u (list (make-mail client (guess u0))) '())))

;; GmNState IWorld CtoSMessage -> [Bundle GmNState [List [Mail IWorld Nat]] Empty]
;; handles a message from the client.
(define (handle-msg u client msg)
  (cond [(string=? (iworld-name client) "Beatrice") 
         (set-theNumber-x! choice (string->number msg))
         (make-bundle u (list (make-mail client (theNumber-x choice))) '())]
        [else
         (define w (next-interval u msg))
         (define theGuess (guess w))
         (cond [(> (interval-small w) (theNumber-x choice))
                (make-bundle w (list (make-mail client -1)) '())]
               [(< (interval-big w) (theNumber-x choice))
                (make-bundle w (list (make-mail client -1)) '())]
               [else (make-bundle w (list (make-mail client theGuess)) '())])]))
