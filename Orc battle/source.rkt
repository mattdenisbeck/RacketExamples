...
#| 
   ...
   orcs, uruk-hai, hydras, slimes, and brigands. They are ready to attack. It is
   ...
   nine keys to play
    ...
    -- With "s", "f", "h", "g", "a", and "b"
    -- the player can 's'tab a specific monster, 
    -- the player may 'f'lail at several monsters; 
    -- the player may 'h'eal herself. 
    -- the player may regenerate stren'g'th
    -- the player may regenerate 'a'gility
    -- the player may 'b'lock, which increases armor
   ...
   Updated 6/26/15 by Matthew Beck
    -- Added actions to regenerate agility and strength. 
    -- Added armor attribute, which absorbs some damage, and block action which 
       increases armor. 
    -- Added new monster type, which is like an orc, but its attacks ignore player's
       armor.
|#
...
(struct uruk-hai monster (club) #:transparent)
...
;; An Uruk-hai is an (uruk-hai Nat Nat Nat)
;; A Slime is a (slime Nat Nat Nat)
;; A Brigrand is a (brigand Nat Nat)
;; A Hydra is a (hydra Nat Nat)    
;; 
;; The five monster types all inherit the id and health fields from monster. 
;; Three have additional attributes: 
;; -- (orc i h c) means the orc's club has strength c
;; -- (uruk-hai i h c) means the uruk-hai's club has strength c
...
(define HEALING 8)
(define STRENGTH-REGEN 4)
(define AGILITY-REGEN 4)
(define ARMOR-REGEN 8)
...
(define INSTRUCTIONS-0
  "Press G to Regenerate strength | Press A to Regenerate agility | Press B to block")
...
(define URUK-HAI(bitmap "graphics/uruk-hai.jpg"))

(define PIC-LIST (list ORC HYDRA SLIME BRIGAND URUK-HAI))
...
(define URUK-HAI-IMAGE(overlay URUK-HAI FRAME))
...
(define INSTRUCTION-TEXT
  (above 
   ...
   (text INSTRUCTIONS-0 (- INSTRUCTION-TEXT-SIZE 4) "blue")))
...
(define (player-acts-on-monsters w k)
  (cond 
    ...
    [(key=? "h" k) (heal w)]
    [(key=? "f" k) (flail w)]
    [(key=? "g" k) (regen-strength w)]
    [(key=? "a" k) (regen-agility w)]
    [(key=? "b" k) (regen-armor w)]
...
(define (initialize-player) 
  (player MAX-HEALTH MAX-AGILITY MAX-STRENGTH START-ARMOR))
...
(define (initialize-monsters)
  ...
  (define (create-monster _)
    ...
    (case (random 5)
      ...
      [(4) (uruk-hai URUK-HAI-IMAGE health (random+ CLUB-STRENGTH))]
...
;; OrcWorld -> Void 
;; Effect: reduces the number of remaining attacks for this turn 
;;   and increases the player's strength level 
(define (regen-strength w)
  (decrease-attack# w)
  (player-strength+ (orc-world-player w) STRENGTH-REGEN))

;; OrcWorld -> Void 
;; Effect: reduces the number of remaining attacks for this turn 
;;   and increases the player's agility level 
(define (regen-agility w)
  (decrease-attack# w)
  (player-agility+ (orc-world-player w) AGILITY-REGEN))

;; OrcWorld -> Void 
;; Effect: reduces the number of remaining attacks for this turn 
;;   and increases the player's armor level 
(define (regen-armor w)
  (decrease-attack# w)
  (player-armor+ (orc-world-player w) ARMOR-REGEN))
    ...
    (cond
      [(orc? monster)
       (if (> (player-armor player) 0)
           (begin (player-armor+ player ARMOR-DAMAGE)
                  (player-health+ player (min 0 (- (random- (orc-club monster)) ARMOR-DAMAGE))))
           (player-health+ player (random- (orc-club monster))))]
      [(uruk-hai? monster)
       (player-health+ player (random- (uruk-hai-club monster)))]
      [(hydra? monster)
       (if (> (player-armor player) 0)
           (begin (player-armor+ player ARMOR-DAMAGE)
                  (player-health+ player (min 0 (- (random- (monster-health monster)) ARMOR-DAMAGE))))
           (player-health+ player (random- (monster-health monster))))]
      [(slime? monster)
       (if (> (player-armor player) 0)
           (begin (player-armor+ player ARMOR-DAMAGE)
                  (player-health+ player (min 0 (- -1 ARMOR-DAMAGE)))
                  (player-agility+ player (random- (slime-sliminess monster))))
           (begin (player-health+ player -1)
                  (player-agility+ player (random- (slime-sliminess monster)))))]
      [(brigand? monster) 
       (case (random 3)
         [(0) (if (> (player-armor player) 0)
                  (begin (player-armor+ player ARMOR-DAMAGE)
                  (player-health+ player (min 0 (- HEALTH-DAMAGE ARMOR-DAMAGE))))
