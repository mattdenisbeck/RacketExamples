...
#| 
   ...
   Modified by Matthew Beck - 7/19/15
   -- Slowed down the A.I. so it takes its moves one by one, with a short delay between
   -- added a "hint" feature, press "h" for the recommended attack destination to be highlighted
      or for the message "Pass" to be displayed. 
|#
...
(struct dice-world (src hint board gt) #:transparent)
;; DiceWorld = (dice-world (U #false Natural) Board GameTree)
;; in (dice-world i j b gt)
...
;; -- if j is a Natural, it is an index for the territory that the player should attack, i.e. hint
;; -- if j is -1 a pass is recommended.
;; -- if j is #f, no hint has been requested
...
(define INSTRUCT 
  "← and → to move among territories, <enter> to mark, <d> to unmark, <p> to pass, and <h> for a hint")
...
(define (roll-the-dice)
  (big-bang (create-world-of-dice-and-doom)             
            ...
            (on-tick ai-plays 1.5) ;;a.i. checks if it is the player every 1.5 sec.
...
(define (create-world-of-dice-and-doom)
  ...
  (define new-world (dice-world #f #f board gamet))
...
(define (refocus-board w direction)
  ...
  (dice-world source #f new-board tree))
...
(define (unmark w)
  (dice-world #f #f (dice-world-board w) (dice-world-gt w)))
...
(define (mark w)
  ...
  (if source 
      ...
      (dice-world focus #f board tree)))

;; DiceWorld -> DiceWorld
;; marks a territory the recommended attack for the player or advises a pass move 
(define (hint w)
  (define tree (dice-world-gt w))
  (define src (dice-world-src w))
  (define board  (dice-world-board w))
  (define ratings  (rate-moves-player tree AI-DEPTH))
  (define the-move (first (argmax second ratings)))
  (define hint (if (empty? (move-action the-move)) -1 (second (move-action the-move))))
  (dice-world src hint board tree))
...
(define (attacking w source target)
  ...
  (if next (dice-world #f #f (game-board next) next) w))
...
;; World hint Scene-> Scene
;; Adds the hint to the scene
(define (add-hint w hint? s)
  (define str "Hint: pass")
  (define txt (text str TEXT-SIZE TEXT-COLOR))
  (if (not hint?)
      s
      (if (= hint? -1) 
          (place-image txt (- WIDTH INFO-X-OFFSET) (+ 40 INFO-Y-OFFSET) s)
          (place-image FOCUS (get-x hint?) (get-y hint?) s))))      
...
(define (pass w)
  (define m (find-move (game-moves (dice-world-gt w)) '()))
  (dice-world #f #f (game-board m) m))

;; DiceWorld -> DiceWorld
;; executes a passing move on the world state
(define (ai-plays w)
  (cond [(= (game-player (dice-world-gt w)) AI)
         (define ai (the-ai-plays (dice-world-gt w)))
         (sleep 1)
         (dice-world #f #f (game-board ai) ai)]
        [else w]))
...
(define (rate-moves-player tree depth)
  (for/list ((move (game-moves tree)))
    (list move (rate-position-player (move-gt move) (- depth 1)))))

;; GameTree Natural -> Number
;; Returns a number that is the best move for the AI player.
...
;; GameTree Natural -> Number
;; Returns a number that is the best move for the human player.
(define (rate-position-player tree depth)
  (cond [(or (= depth 0) (no-more-moves? tree))
         (define-values (best w) (winners (game-board tree)))
         (if (member HUMAN w) (/ 1 (length w)) 0)]
        [else 
         (define ratings (rate-moves tree depth))
         (apply (if (= (game-player tree) HUMAN) max min)
                (map second ratings))]))
