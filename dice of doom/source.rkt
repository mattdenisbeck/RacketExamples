...
#| 
   ...
   Modified by Matthew Beck - 7/13/2015
   - attacks are now determined by a random roll of the dice,
     if the attacker loses the roll, all but one of the dice are removed from that territory
   - fixed ending game text, now says "A.I. Won" when it does
|#
...
(struct move (action gt-succeed gt-fail) #:transparent)
;; Move = (move Action GameTree)
;; in (move a gt-succeed gt-fail)
;; -- a represents the action to be taken
;; -- gt-succeed is the game-tree resulting from a winning roll on that action 
;; -- gt-fail is the game-tree resulting form a losing roll on that action
...
;; Integer -> Integer
;;returns a random roll of num dice
(define (roll-dice num)
  (for/sum ([i num])
    (+ 1 (random 6))))

;; [Natural Natural Board] -> Integer (ie the territory index)
;;returns the index of a winning territory from an attack, ties go to the defender
(define (dice-battle src dst board)
  (define source (get-territory src board))
  (define destination (get-territory dst board))
  (define src-roll (roll-dice (territory-dice source)))
  (define dst-roll (roll-dice (territory-dice destination)))
  (cond [(> (begin (display "Attacker roll: ") (displayln src-roll) src-roll) 
            (begin (display "Defender roll: ") (displayln dst-roll) dst-roll)) 
         (territory-index source)]
        [else (territory-index destination)]))

;; [Natural Board] -> Territory
;; returns territory from index
(define (get-territory indx board)
  (for/first ([t board]
              #:when (= indx (territory-index t)))
     t))
...
(define (won board)
  ...
  (if (cons? (rest w)) "It's a tie." 
      (if (= 0 (first w)) "You won." "The A.I. won.")))
...
(define (game-tree board player dice)
  ;; create tree of attacks from this position (both winning roll and losing roll trees); add passing move
  (define (attacks board) 
    (for*/list ([src board] 
      ...
      (define succeed (execute board player from dst dice))
      (define fail (suffer-defeat board player from dst dice))
      (define attacks-from-succeed 
        (game succeed player (cons (passes succeed) (attacks succeed))))
      (define attacks-from-fail 
        (game fail player (cons (passes fail) (attacks fail))))
      (move (list from dst) attacks-from-succeed attacks-from-fail)))
  ;; create a passing move , distribute dice, continue
  (define (passes board)
    (define-values (new-dice newb) (distribute board player dice))
    (move '() (game-tree newb (switch player) new-dice) (game-tree newb (switch player) new-dice)))
  ;; -- START: -- 
...
;; Creates a new board after a winning attack
...
;; Board Natural Natural Natural Natural -> Board
;; Creates a new board after a defeated attack
;; updates only src and dst
(define (suffer-defeat board player src dst dice)
  (for/list ([t board])
    (define idx (territory-index t))
    (cond [(= idx src) (territory-set-dice t 1)]
          [else t])))
...
(define (attacking w source target)
  ...
  (define next     (find-move feasible attack w))
...
;; randomly determines winning roll or losing roll based on "dice-batle" function
;; return succeed or fail game tree as appropriate
(define (find-move moves a w)
  (define m (findf (lambda (m) (equal? (move-action m) a)) moves))
  (define board (dice-world-board w))
  (if (null? a) (and m (move-gt-succeed m)) 
  (if (= (dice-battle (first a) (second a) board) (car a)) 
      (and m (move-gt-succeed m)) 
      (and m (move-gt-fail m)))))
...
;; THIS REQUIRES A DIFFERENT DEFINITION FOR PLAIN CHAPTER 10.
(define (pass w)
  (define m (find-move (game-moves (dice-world-gt w)) '() w))
  (cond [(not m) w]
...
(define (the-ai-plays tree)
  ...
  (define the-board (game-board tree))
  (define new-tree (if (null? (move-action the-move)) (move-gt-fail the-move) (if (= (dice-battle (first(move-action the-move)) (second (move-action the-move)) the-board) (first (move-action the-move))) 
       (move-gt-succeed the-move) 
       (move-gt-fail the-move))))
...
;; (if (= (dice-battle (first(move-action the-move)) (second (move-action the-move)) board) (first (move-action the-move))) 
;;        (move-gt-succeed the-move) 
;;        (move-gt-fail the-move))

;;(define new-tree (move-gt-succeed the-move))
...
(define (rate-moves tree depth)
  (for/list ((move (game-moves tree)))
    (list move (rate-position (move-gt-succeed move) (- depth 1)))))
