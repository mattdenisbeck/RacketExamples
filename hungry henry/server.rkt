...
(define (move-and-eat pu)
  ...
  (define nfood (new-food (feed-em-all nplayers (play-food pu)) COUNT))
  (progress nplayers nfood (play-spectators pu)))

(define (new-food newFoodList count)
  (define difference (- (count-cupcakes count) (length newFoodList)))
  (cond [(= difference 2)
         (set-count-cupcakes! COUNT (- (count-cupcakes count)  1))
         (cons (create-a-body CUPCAKE) newFoodList)]
        [else newFoodList]))
